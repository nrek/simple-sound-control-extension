...
- **`srcObjectElements` set generalised to `volumeFallback`.** Same volumechange-enforcer machinery now serves three populations: WebRTC (`srcObject`) elements, cross-origin-tainted elements, and elements where the page won the `createMediaElementSource` race. Boost > 100% remains gain-node-only (and therefore unavailable on the fallback path); Tab Capture mode covers boost for these cases.
- **`routeAll` no longer eagerly constructs the `AudioContext`.** Graph construction moved into `tryRoute` so a tab whose only media elements all land on the fallback path never allocates an `AudioContext` it can't use.
- **Iteration cruft removed from the content script.** Dropped (a) the `routeFailed` `WeakSet` — every `createMediaElementSource` failure now falls back to `el.volume` (strictly better than going inert; the typed `InvalidStateError` branch added earlier in 0.2.3 generalised to the same path), (b) the `volumeFallbackExpected` `WeakMap` and its secondary short-circuit in the enforcer — the `|el.volume − desired|` check is already a sufficient loop guard since our own writes always land at `desired`, the throttle that originally motivated `expected` was removed in 0.1.9, and (c) the redundant secondary `srcObject` block in `attachMedia` — `tryRoute` now classifies `srcObject` itself once `audioUnlocked` is true, and the pre-activation attenuation window the old block covered is negligible.
